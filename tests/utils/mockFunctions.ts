...
export class SubstrateTestEventBuilder<T extends AnyTuple = AnyTuple> {
  ...
  buildEventForPoolReward(rewardAccountId, poolId, amount): SubstrateEvent<T> {
    return this.withBlock()
      .withEvent([rewardAccountId, poolId, amount])
      .build();
  }
  ...
  buildEventForUnbondingPoolSlash(poolId, era, amount): SubstrateEvent<T> {
    return this.withBlock().withEvent([poolId, era, amount]).build();
  }
}
