...
export class CollatorStakingRewardCalculator implements RewardCalculator {
  ...
  async maxApy(): Promise<number> {
    ...
    let apr = stakingDeviation
      .mul(stakedInfo.totalIssuance)
      .mul(inflation)
      .div(stakedInfo.totalStaked);
  ...
  private async fetchInflationDistributionInfo(): Promise<number> {
    const inflationDistributionInfo =
      await api.query.parachainStaking.inflationDistributionInfo();
