...
export class NominationPoolRewardCalculator implements RewardCalculator {
  ...
  constructor(
    ...
    relayChainRewardCalculator: ValidatorStakingRewardCalculator,
  ) {
  ...
  async maxApy(): Promise<number> {
    if (
      api.query["nominationPools"] === undefined ||
      typeof api.query.nominationPools["bondedPools"] !== "function"
    ) {
      return 0;
    }
    ...
    const poolsCommission = new Map<string, number>(
      await Promise.all(
        pools.map(async ([poolId, poolDataOption]) => {
          const poolIdNumber = (
            poolId.args[0] as unknown as INumber
          ).toNumber();
          const poolAddress = this.derivePoolAccount(
            poolIdNumber,
            0,
          ).toString();
          ...
          const poolData = (
            poolDataOption as unknown as {
              unwrap: () => {
                commission?: {
                  current?: { isSome: boolean; unwrap: () => [Perbill] };
                };
              };
            }
          ).unwrap();
          const currentCommission = poolData.commission?.current;
          if (currentCommission?.isSome) {
            poolCommission = PerbillToNumber(currentCommission.unwrap()[0]);
          }
    ...
    const maxApy = eraStakers.reduce((accumulator, stakeTarget) => {
      const stakerApy = stakersApy.get(stakeTarget.address) ?? 0;

      return Math.max(
        accumulator,
        stakeTarget.others.reduce((innerAccumulator, staker) => {
          if (poolsCommission.has(staker.address)) {
            return Math.max(
              innerAccumulator,
              (1 - poolsCommission.get(staker.address)) * stakerApy,
            );
          } else {
